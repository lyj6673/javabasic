...
public class HomeWork18 {
	...
	public void process() {
		...
		// 1 최대 월급 구하기
		System.out.println("월급총합 : "+maxSalary(list));
		
		// 2 각 부서별 월급 총합
		Map<String, Integer> deptSum = deptSum(list);
		...
		for(Map map : list) {
			if(map.get("emp_no").equals(101)) {
				System.out.println(map.get("emp_no")+"\t"+map.get("emp_name")+
						"\t"+map.get("dept")+"\t"+map.get("salary"));
			}
		}
		
	}
	public void printEmpNo101(List<Map> list) {
		for(Map map : list) {
			int emp_no = (int)map.get("emp_no");
			if(emp_no != 101)continue;
			System.out.println(map.get("emp_no")+"\t"+map.get("emp_name")+
					"\t"+map.get("dept")+"\t"+map.get("salary"));
		}
	...
	public Map<String, Integer> deptSum(List<Map> list) {
		...
		for(Map map : list) {
			...
			if(result.containsKey(dept))salary+=result.get(dept);
			result.put(dept,salary);
		}
		return result;
